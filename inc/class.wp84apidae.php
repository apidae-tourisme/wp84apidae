	...
	public static function do_admin_notice() {
		if ( get_transient( 'wp84apidae_msg_status' ) === 'dir_tmp_error' ) {
			...
			<div class="notice notice-warning is-dismissible">
				<p>Ce plugin a été développé sur php 5.6 et Wordpress 4.7. Votre version de php et/ou Wordpress semble
					antérieure, le plugin pourrait ne pas fonctionner correctement...</p>
				<?php
	...
	public static function apidaelist_shorttag( $atts ) {
		if ( array_key_exists( 'list', $atts ) && filter_var( $atts['list'], FILTER_VALIDATE_INT ) && array_key_exists( 'detail', $atts ) && array_key_exists( 'templist', $atts ) && filter_var( $atts['detail'], FILTER_VALIDATE_INT ) && filter_var( $atts['templist'], FILTER_VALIDATE_INT ) ) {
			...
			if ( count( $listpar ) > 0 && count( $tlistpar ) > 0 ) {
				...
				$sMoteur   = '';
				$oObj      = null;
				$sQuery    = get_query_var( 'apisearch', '' );
				$aParams   = get_query_var( 'apicritere', '' ) != '' ? explode( '/', get_query_var( 'apicritere', '' ) ) : array();
				...
				if ( ! empty( $sQuery ) ) {
					if ( ! empty( $parbase['searchQuery'] ) ) {
						$parbase['searchQuery'] .= ' ' . $sQuery;
					} else {
						$parbase['searchQuery'] = $sQuery;
					}
				}
				...
				if ( $oObj !== null ) {
					...
					if ( ! empty( $parbase['criteresQuery'] ) ) {
